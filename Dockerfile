...
RUN GO111MODULE=on go test -v \
    -ldflags "-X github.com/unhookd/unctl/config.EncodedConfigLookup=$(cat zero-trust-test.yaml | base64 -w0)" \
    ./...

RUN GO111MODULE=on GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build \
    -ldflags "-X github.com/unhookd/unctl/config.EncodedConfigLookup=$(cat zero-trust.yaml | base64 -w0)" \
